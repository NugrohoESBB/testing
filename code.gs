...
// DEVELOPMENT 2 (fix)
const SPREADSHEET_ID = "1FobYporDFu7_uQa3o-v9vT6PQY1oEYDoy5ZB6WiqZJI";
const DRIVE_FOLDER_ID = "1Y9tWDAWdJnUvT_efiCiOW5eMWhRrx-i_";

/**
 * @OnlyCurrentDoc
 */

// Paksa Apps Script minta izin Drive
function authorizeDrive() {
	DriveApp.getRootFolder(); // dummy call for trigger OAuth
}

function authorizeAll() {
	// Trigger OAuth untuk Sheets
	SpreadsheetApp.openById(SPREADSHEET_ID);
	// Trigger OAuth untuk Drive
...
function doGet(e) {
	...
	try {
		...
		else if (action === "updateStatus") result = updateOrderStatus(e.parameter);
		else if (action === "saveMenu") {
			const item = JSON.parse(e.parameter.item);
			result = saveMenuItem({ item });
		} else if (action === "uploadFoto") {
			// ← tambah ini
			result = uploadFoto(e.parameter);
		} else if (action === "deleteMenu") {
			// ← tambah ini
			result = deleteMenuItem(e.parameter);
		} else result = { error: "Unknown action" };
	} catch (err) {
		result = { error: err.toString() };
	}
...
function doPost(e) {
	...
	try {
		...
		else if (action === "uploadFoto") result = uploadFoto(payload);
		else result = { error: "Unknown action" };
	} catch (err) {
		result = { error: err.toString() };
	}
...
function uploadFoto(payload) {
	try {
		...
		// Decode base64 jadi blob
		const blob = Utilities.newBlob(Utilities.base64Decode(payload.base64), payload.mimeType, payload.fileName);

		// Upload ke Drive
		const file = folder.createFile(blob);

		// Set permission publik agar bisa diakses dari index.html
		file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);

		// Format URL langsung bisa ditampilkan di <img>
		const fileId = file.getId();
		//const publicUrl = `https://drive.google.com/uc?export=view&id=${fileId}`;
		const publicUrl = `https://drive.google.com/thumbnail?id=${fileId}&sz=w800`;
...
function deleteMenuItem(payload) {
	const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
	const sheet = ss.getSheetByName("Menu");
	if (!sheet) return { error: "Sheet Menu tidak ditemukan" };

	const rows = sheet.getDataRange().getValues();
	const rowIdx = rows.findIndex((r, i) => i > 0 && String(r[0]) === String(payload.id));

	if (rowIdx < 0) return { error: "Item tidak ditemukan" };
